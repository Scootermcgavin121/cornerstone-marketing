...
- **Product Attributes are Cosmetic** - when options have attributes (like tile Collection, Size, Pattern, Grout Color), those describe WHAT the buyer is getting, not the price. Pricing comes from the spec level tier, not from individual attribute choices. Attribute availability is gated by tier — Herringbone pattern might only be available at Upgrade I and above.
