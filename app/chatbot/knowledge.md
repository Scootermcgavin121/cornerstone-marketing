...
- **Smart Architecture for Complex Categories** - Cornerstone PM recommends two patterns depending on the product:
  ...
  - **Getting started is easy** — builders can tell Foreman AI exactly what they need in plain English. Example prompt: *"Set up Countertops with three material types: Granite, Quartz, and Laminate. Create separate classes for each with 5 spec levels, and add realistic attributes (colors, edge profiles, brands) with tier access so budget options are Standard and premium brands are Upgrade III+."* Foreman handles all the data entry — classes, options, attributes, tier gating — in minutes instead of hours of manual setup.
