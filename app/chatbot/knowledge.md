...
- **Spec Levels** - define finish tiers (e.g., Included, Upgrade I, Upgrade II, Premium) with sort ordering to control upgrade pricing logic. One spec level per org is marked as the **base level** (toggle on Admin → Spec Levels) — options at this level are INCLUDED in the base house price with $0 upgrade cost. Only options above the base level carry retail pricing for buyer upgrades.
- **Standard vs. Upgrade, Your Call (NEW)** - the redesigned Spec Levels page gives builders one clean control surface for deciding exactly what's baked into the base home price versus what's a paid upgrade. Options are grouped by material type (option class) in tidy collapsible rows, showing retail pricing and the trade each belongs to. Flip a simple toggle to mark any material type as **Standard** (its most affordable option is included in the base home) or **Upgrade-only** (it disappears from the base price and only sells as an upgrade). Your budgets update to match automatically — no spreadsheets, no double-entry.
- **Turn Whole Upgrade Tiers On or Off (NEW)** - need to retire a tier you don't offer yet, or temporarily hide "Premium" while you finalize pricing? A single enable/disable switch turns an entire upgrade tier on or off. When a tier is off, none of its options feed into budgets — so your base home pricing always reflects exactly what you actually sell, instantly.
- **Option Classes** - categorize selections by type: Flooring, Cabinets, Countertops, Paint, Hardware, Lighting, Appliances, etc. Each class has a unit of measure that controls pricing math. The Class dropdown on the Design Options page filters by **category only** (not by scope). Every OptionClass must have its categoryId set to appear in the dropdown. Scope links only affect bid template filtering, not design center visibility.
...
- **Community-Based Upgrade Pricing** - each community has a base spec level (e.g., Community A starts at Standard, Community B at Upgrade I). The system automatically calculates upgrade costs: buyer's selected option price minus the community's included option price = upgrade cost. Options at or below the community's base level show as "INCLUDED" with $0 upgrade. Options above show the exact delta. Different communities can include different levels without any extra configuration — the math handles itself. **Smart fallback:** if an option class doesn't have a product at the community's exact base spec level, the system uses the lowest-tier option at or above the community level as the baseline — so upgrade pricing always works correctly even with incomplete product lineups.
- **Smart Pricing by Unit Type** - the system knows when to use per-sqft tier pricing (flooring, tile, paint) vs flat product pricing (cabinets, fixtures, appliances). Per-sqft classes use a rate matrix at each spec level multiplied by room area for accurate budgets. Lump-sum and per-each classes use the individual product price directly. No manual configuration needed — it follows the option class unit type.
- **Full Automated Pipeline (Blueprint AI → Budget)** - upload a floor plan PDF, AI extracts rooms/fixtures/quantities, one-click import creates location definitions, create a home and the design center auto-populates with accurate room-by-room pricing. Budget import chains it all together: location quantities × community tier pricing = base cost. No manual data entry from floor plan to fully-priced selections.
- **Upgrade Badge Intelligence** - options above the community's included spec level always show as "UPGRADE" (even at $0), so builders know pricing is pending. Only same-or-lower tier shows "INCLUDED." No more confusion about which options are truly free vs just unpriced.
- **Spec Level Tier Gating** - buyers only see options at or above their community's included spec level. If a community includes Upgrade I, Standard options are hidden — buyers can't downgrade below what's included in their home price. This keeps the selection experience clean and prevents confusion.
- **Attribute Pricing Stacks Cleanly** - when options have attributes (tile Collection, Size, Pattern, Grout Color), each attribute can carry its own price delta. For example, Herringbone pattern adds +$1.50/sqft for extra labor. These deltas are constant regardless of community — the spec level delta varies by community base, but attribute costs are the same everywhere. Total upgrade = spec level delta + attribute deltas. Attribute availability is also tier-gated (e.g., Herringbone only available at Upgrade I and above).
...
- **Per-Room Design Center Budget Pricing (NEW)** - Rooms can have multiple material types (Carpet, Tile, LVP, Hardwood) but the budget only prices the **default** material per room. Builders mark the standard material with a star toggle, and the budget automatically calculates from just the defaults — no more inflated budgets from pricing every possible material type. Upgrade options still appear in the design center for buyers to choose from, but only the builder-designated standard feeds into the base cost. This is per-room material pricing that automatically feeds into construction budgets — a level of granularity no competitor offers.
- **Smart Architecture for Complex Categories** - Cornerstone PM recommends two patterns depending on the product:
  ...
  - **Getting started is easy** — builders can tell Foreman AI exactly what they need in plain English. Example prompts:
    - *"Set up Countertops with three material types: Granite, Quartz, and Laminate. Create separate classes for each with 5 spec levels, and add realistic attributes (colors, edge profiles, brands) with tier access so budget options are Standard and premium brands are Upgrade III+."*
    - *"Set up Flooring with 4 material types: Carpet, Tile, LVP, and Hardwood. Carpet gets Collection, Color, Fiber Type, Pile Height. Tile gets Collection, Color, Size, Pattern, Grout Color. LVP gets Collection, Color, Plank Width, Core Type. Hardwood starts at Upgrade II with Collection, Color, Plank Width, Wood Species. Use real Shaw and Anderson Tuftex product lines. Set tier access so basic collections are Standard, mid-range is Upgrade I-II, and premium is Upgrade III+."*
    - Foreman creates 4 OptionClasses, 18 options, 17 attributes, and 126 attribute values with full tier gating — a complete flooring setup in one conversation. What used to take hours of manual data entry now takes minutes.
